...
async fn get_data(
    ...
    if let Some(payment_proof) = req.headers().get("X-402-Payment-Proof") {
        ...
        if paid.contains(&tx_id.to_string()) {
            return Ok(HttpResponse::PaymentRequired().json(PaymentRequiredResponse {
        ...
        return Ok(HttpResponse::Ok().json(DataResponse {
            data: "Protected content accessed successfully! 🎉".to_string(),
...
async fn main() -> std::io::Result<()> {
    ...
    println!("🚀 Starting MCP Server with x402 Payments...");
    println!("📡 Server running at: http://localhost:8402");
    println!("💰 Payment endpoint: http://localhost:8402/data");
    println!("🏥 Health check: http://localhost:8402/health");
    println!("\nℹ️  Make sure x402-dev daemon is running:");
