...
fn arbitrary_policy_action() -> impl Strategy<Value = PolicyAction> {
    prop_oneof![
        ...
        (1u32..1000).prop_map(|rpm| PolicyAction::RateLimit {
...
fn arbitrary_policy_defaults() -> impl Strategy<Value = PolicyDefaults> {
    (arbitrary_policy_action(), 1u64..3600).prop_map(|(default_action, timeout_seconds)| {
        PolicyDefaults {
            default_action,
            timeout_seconds,
        }
...
fn arbitrary_request_context() -> impl Strategy<Value = RequestContext> {
    (
        ...
        .prop_map(
            |(endpoint, method, user_id, payment_status, request_count)| RequestContext {
                endpoint,
                method,
                user_id,
                payment_status,
...
fn arbitrary_payment_status() -> impl Strategy<Value = PaymentStatus> {
    prop_oneof![
        ...
        ("[a-zA-Z0-9]{32}", 1u64..1_000_000)
            .prop_map(|(tx_id, amount)| { PaymentStatus::Paid { tx_id, amount } }),
    ]
}
...
fn generate_code(policy: &PolicyConfig) -> String {
    let mut code = format!("// Generated policy code v{}\n\n", policy.version);
    ...
    for rule in sorted_rules {
        if evaluate_condition(&rule.condition, request) {
            return match &rule.action {
                ...
                PolicyAction::RequirePayment { amount } => match &request.payment_status {
                    PaymentStatus::Paid {
                        amount: paid_amount,
                        ..
