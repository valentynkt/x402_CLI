...
fn arbitrary_valid_invoice() -> impl Strategy<Value = X402Invoice> {
    (valid_amount(), valid_base58_address(), valid_network()).prop_map(|(amount, address, _network)| {
...
proptest! {
    ...
    fn minimum_amount_validates() {
        assert!(validate_amount(1), "Minimum amount (1 lamport) should validate");
    }

    #[test]
    fn maximum_amount_validates() {
        let max_amount = 1_000_000_000_000u64; // 1000 SOL
        assert!(validate_amount(max_amount), "Maximum amount should validate");
    }
}
...
mod boundary_conditions {
    ...
    fn minimum_address_length_validates() {
        ...
        assert!(validate_base58(&address), "32-character address should validate");
    }

    #[test]
    fn maximum_address_length_validates() {
        let address = "1".repeat(44);
        assert!(validate_base58(&address), "44-character address should validate");
    }

    #[test]
    fn below_minimum_length_fails() {
        let address = "1".repeat(31);
        assert!(!validate_base58(&address), "31-character address should fail");
    }

    #[test]
    fn above_maximum_length_fails() {
        let address = "1".repeat(45);
        assert!(!validate_base58(&address), "45-character address should fail");
    }
}
...
mod tests {
    ...
    fn test_basic_validation() {
        ...
        assert_eq!(validate_invoice(&invalid_amount), ValidationResult::InvalidAmount);
        ...
        assert_eq!(validate_invoice(&invalid_address), ValidationResult::InvalidBase58);
    }
}
