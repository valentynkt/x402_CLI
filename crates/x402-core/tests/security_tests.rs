...
fn test_future_timestamp_attack_rate_limit() {
    ...
    assert!(
        state.check_limit(window, max_requests, now),
...
fn test_massive_future_timestamp_flood() {
    ...
    assert_eq!(
        state.count_in_window(window, now),
        5,
...
fn test_cleanup_removes_invalid_timestamps() {
    ...
    assert_eq!(
        valid_count, 1,
        "Only valid timestamp should remain after cleanup"
    );
}
...
fn test_check_limit_count_in_window_consistency() {
    ...
    assert_eq!(
        has_capacity, expected_has_capacity,
...
fn test_zero_window() {
    ...
    assert_eq!(
        count, 1,
        "Zero window should only count exact timestamp matches"
    );
}
